...
public class PasswdDelegatorTest extends AbstractP4JavaUnitTest {
    ...
    private void executeAndVerify(String expectedNewPassword2String,
            String expectedOldPasswordString, String expectedNewPasswordString) throws Exception {

        Map<String, Object> pwdMap = new HashMap<String, Object>();
        pwdMap.put(NEW_PASSWORD2, expectedNewPassword2String);
        pwdMap.put(OLD_PASSWORD, expectedOldPasswordString);
        pwdMap.put(NEW_PASSWORD, expectedNewPasswordString);

        when(server.execMapCmdList(eq(PASSWD.toString()), eq(new String[] { userName }),
                eq(pwdMap))).thenReturn(buildChangePasswordList());
