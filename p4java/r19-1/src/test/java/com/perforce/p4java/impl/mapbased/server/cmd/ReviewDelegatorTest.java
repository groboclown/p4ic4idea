...
public class ReviewDelegatorTest extends AbstractP4JavaUnitTest {
    ...
    /**
     * Expected return empty 'review' list when 'create review changelist'
     * throws exception
     *
     * @throws Exception
     */
    ...
    public void shouldThrowsExceptionWhenInnerMethodCallThrowsIt() throws Exception {
        ...
        // given
        doThrow(AccessException.class).when(server).execMapCmdList(eq(REVIEW.toString()),
                eq(CMD_ARGUMENTS), eq((Map<String, Object>) null));
        // when
    ...
    private List<IReviewChangelist> executeCmdAndReturn() throws Exception {
        // given
        when(server.execMapCmdList(eq(REVIEW.toString()), eq(CMD_ARGUMENTS),
