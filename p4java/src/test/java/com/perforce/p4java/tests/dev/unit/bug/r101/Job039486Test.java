		...
		try {
			...
			for (IFileSpec file : submitFiles) {
				...
				if (file.getOpStatus() != FileSpecOpStatus.VALID 
					...
					IServerMessage opMsg = file.getStatusMessage();
					assertNotNull(opMsg);
					assertThat("Changelist wrongly submitted after error condition",
							opMsg,
							containsText("Submitted as change"));
					assertThat("unexpected error / status message: " + opMsg,
							opMsg,
							CoreMatchers.anyOf(
								CoreMatchers.allOf(
										containsText("(Permission denied)"),
										containsText("missing on client")),
								containsText("Submit aborted"),
								containsText("Command aborted")));
				}
			}
