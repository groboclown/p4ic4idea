...
public class FileAnnotateDelegatorTest extends AbstractP4JavaUnitTest {
    ...
    /**
     * Expected thrown <code>RequestException</code> when "non whitespace diff
     * option" of <code>DiffType</code> given.
     *
     * @throws Exception
    ...
    public void shouldThrowRequestExceptionWhenNonWhitespaceDiffTypeGiven() throws Exception {
        ...
        verify(server, never()).execMapCmdList(eq(ANNOTATE.toString()), any(String[].class),
    ...
    /**
     * Expected thrown <code>ConnectionException</code> when inner
     * getFileAnnotations() throws the exception.
     *
     * @throws Exception
     */
    @Test(expected = ConnectionException.class)
    public void shouldThrowExceptionWhenInnerGetFileAnnotationsThrowsIt() throws Exception {
        doThrow(ConnectionException.class).when(server).execMapCmdList(eq(ANNOTATE.toString()),
                any(String[].class), eq((Map<String, Object>) null));
        executeGetFileAnnotations(whitespaceDiffType);
    }
    ...
    public void shouldReturnNonEmptyFileAnnotations() throws Exception {
        mockResultMaps();
        doReturn(resultMaps).when(server).execMapCmdList(eq(ANNOTATE.toString()),
                any(String[].class), eq((Map<String, Object>) null));
    ...
    private List<IFileAnnotation> executeGetFileAnnotations(DiffType diffType) throws Exception {
        ...
        return fileAnnotateDelegator.getFileAnnotations(fileSpecs, diffType, allResults,
                useChangeNumbers, followBranches);
    }
    ...
    public void shouldReturnEmptyFileAnnotatesWhenResultMapIsNull() throws Exception {
        when(server.execMapCmdList(eq(ANNOTATE.toString()), any(String[].class),
    ...
    private void executeThenVerifyEmptyFileAnnotations() throws P4JavaException {
        List<IFileAnnotation> fileAnnotations = fileAnnotateDelegator.getFileAnnotations(fileSpecs,
    ...
    private void mockResultMaps() {
        ...
        resultMaps = new ArrayList<Map<String, Object>>();
        resultMaps.add(resultMap0);
        resultMaps.add(resultMap1);
        resultMaps.add(newDepotFileResultMap1);
    }
}