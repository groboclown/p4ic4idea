...
public class ClientConfigConnectionFailedMessage extends ProjectMessage<ClientConfigConnectionFailedMessage.Listener> {
    ...
    public interface Listener {
        ...
        void connectionRefused(@NotNull ClientConfig config);
    }
    ...
    public static class PostEventListener implements Listener {
        ...
        @Override
        public void connectionRefused(@NotNull ClientConfig config) {
            if (canSendMessage(project)) {
                project.getMessageBus().syncPublisher(TOPIC).connectionRefused(config);
            }
        }
    }
     ...
     * problem was, but only that there was a problem.
     */
    public static abstract class HostErrorListener
            implements Listener {

        public abstract void onLoginError(@NotNull ClientConfig config);

        public abstract void onHostConnectionError(@NotNull ClientConfig config);

        @Override
        public void connectionTimedOut(@NotNull ClientConfig config) {
            onHostConnectionError(config);
        }
