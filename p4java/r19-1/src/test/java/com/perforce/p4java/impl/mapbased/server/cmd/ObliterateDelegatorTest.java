...
public class ObliterateDelegatorTest extends AbstractP4JavaUnitTest {
    ...
    private void expectedReturnEmptyList(UnitTestGiven unitTestGiven) throws Exception {
        //given
        unitTestGiven.given();
        when(server.execMapCmdList(eq(OBLITERATE.toString()), eq(CMD_ARGUMENTS), eq((Map<String, Object>)null))).thenReturn(resultMaps);
        //when
    ...
    private void shouldReturnNonEmptyList(
        ...
        //given
        when(server.execMapCmdList(eq(OBLITERATE.toString()), eq(CMD_ARGUMENTS), eq((Map<String, Object>)null)))
