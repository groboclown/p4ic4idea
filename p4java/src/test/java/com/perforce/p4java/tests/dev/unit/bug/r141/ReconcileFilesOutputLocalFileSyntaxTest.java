...
/**
 * Test 'p4 reconcile -l' with output in local file syntax.
 */
@Jobs({"job054780"})
@TestId("Dev141_ReconcileFilesOutputLocalFileSyntaxTest")
public class ReconcileFilesOutputLocalFileSyntaxTest extends P4JavaRshTestCase {

	@ClassRule
	public static SimpleServerRule p4d = new SimpleServerRule("r16.1", ReconcileFilesOutputLocalFileSyntaxTest.class.getSimpleName());
	 ...
	 */
	@Test
	public void testReconcileFilesOutputLocalFileSyntax() {
		...
		String testZipFile = client.getRoot() + "/112Dev/GetOpenedFilesTest/src/com/perforce/p4cmd2/dir1.zip";

		String deleteFile = client.getRoot() + "/112Dev/GetOpenedFilesTest/src/gnu/getopt2/LongOpt.java";

		try {
			files = client.sync(FileSpecBuilder.makeFileSpecList(new String[]{testZipFile, deleteFile}),
			...
			changelist = getNewChangelist(server, client, "Dev141_ReconcileFilesOutputLocalFileSyntaxTest add files");
			...
			files = client.reconcileFiles(
					...
					new ReconcileFilesOptions().setChangelistId(changelist.getId()).setLocalSyntax(true).setLocalSyntax(true));
			...
			for (IFileSpec f : files) {
				if (f.getOpStatus() == FileSpecOpStatus.INFO) {
					...
					if (doesMessageContainText(f.getStatusMessage(),
					...
					assertNotNull(f.getLocalPath());
					System.out.println("LOCAL: " + f.getLocalPath());
