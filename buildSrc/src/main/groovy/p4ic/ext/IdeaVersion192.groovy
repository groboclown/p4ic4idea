...
class IdeaVersion192 implements IdeaVersionLibMatcher {
    private static final Map<String, List<String>> INTELLIJ_192_SPECIAL_MAPPINGS = [
    ...
    private static final Pattern IDEA_VERSION = Pattern.compile("^19[12]\$")
    ...
    String[] matches(final String filename, @Nonnull final Collection<String> choices) {
        ...
        if (choices.contains(shortName)) {
            if (shortName == "annotations") {
                // special case
                return ["annotations-common", "annotations"]
            }
            return [shortName]
        }
