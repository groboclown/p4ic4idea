...
public class DepotsDelegatorTest extends AbstractP4JavaUnitTest {
    ...
    public void testGetDepots() throws P4JavaException {
        when(server.execMapCmdList(eq(DEPOTS.toString()), argThat(EMPTY_MATCHER), eq((Map<String,Object>)null)))
                .thenReturn(buildDepotsMap());
        List<IDepot> result = depotsDelegator.getDepots();
        verify(server).execMapCmdList(eq(DEPOTS.toString()), argThat(EMPTY_MATCHER), eq((Map<String,Object>)null));
