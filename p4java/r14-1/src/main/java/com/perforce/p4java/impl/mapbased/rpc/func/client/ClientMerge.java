...
public class ClientMerge {
	...
	protected RpcPacketDispatcherResult clientOpenMerge3(RpcConnection rpcConnection,
		...
		if (cmdEnv.getCmdSpec().getInMap() != null) {
			...
			mergeState = new ClientMergeState(clientPath, true, clientType, resultType,
			...
			mergeState = new ClientMergeState(clientPath, false, clientType, resultType,
												this.tmpDirName, rpcConnection.getClientCharset());
			mergeState.setTwoWayMerge(twoWayMerge);
			...
			try {
				...
				if (twoWayMerge) {
					mergeState.setYourDigest(
							new MD5Digester().digestFileAs32ByteHex(new File(clientPath), null, false));
				}
			...
			if (skip) {
				...
				if ((clientPath != null) && (tmpFileName != null)) {
					...
					if (!tmpFile.renameTo(new File(clientPath), true)) {
						...
						digest = new MD5Digester().digestFileAs32ByteHex(tmpFile,
															rpcConnection.getClientCharset(), true);
				...
				if (mergeState.isTwoWayMerge()) {
					...
					String yourDigest = mergeState.getYourDigest();	// ditto
