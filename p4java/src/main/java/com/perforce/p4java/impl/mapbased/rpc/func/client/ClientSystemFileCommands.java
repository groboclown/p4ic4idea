...
public class ClientSystemFileCommands {
	...
	protected RpcPacketDispatcherResult openFile(RpcConnection rpcConnection,
		...
		try {
			...
			if (targetFile.exists() && !cmdEnv.isSyncInPlace()) {
			...
			try {
				if (tmpFile != null) {
					RpcOutputStream tmpStream = new RpcOutputStream(tmpFile, rpcConnection, useLocalDigester);
					if (useLocalDigester) {
						tmpStream.setServerDigest(digest);
					}
					handler.getMap().put(FILE_OPEN_TMP_STREAM_KEY, tmpStream);
				} else if (targetFile != null) {
					RpcOutputStream targetStream = new RpcOutputStream(targetFile, rpcConnection, useLocalDigester);
					if (useLocalDigester) {
					...
					try {
						...
						cmdEnv.handleResult(
								new RpcMessage(
										...
										MessageSeverityCode.E_FAILED,
										MessageGenericCode.EV_CLIENT,
										new String[]{targetFile.getName(), e.getLocalizedMessage()}
								).toMap()
						);
		...
		if (clientType != null) {
			...
			if (fileSize != null) {
				try {
					...
					overSize = file.length() > checkSize;
				} catch (NumberFormatException e) {
				}
			}
