...
public class EditFilesOptionsTest {
	...
	public void optionsSpecs() throws Throwable {
		Assert.assertEquals("b:n b:k i:c:gtz s:t s:Q", OPTIONS_SPECS);
	}
	...
	private static void testMethod(List<IFileSpec> fileSpecs, boolean useOldMethod) throws Throwable {
		Assert.assertEquals(valids.immutable, editFilesOptions.isImmutable());
		Assert.assertEquals(valids.noUpdateGet, editFilesOptions.isNoUpdate());
		Assert.assertEquals(valids.changelistIdGet, editFilesOptions.getChangelistId());
		Assert.assertEquals(valids.fileTypeGet, editFilesOptions.getFileType());
		Assert.assertEquals(valids.bypassClientUpdateGet, editFilesOptions.isBypassClientUpdate());
		...
		if (valids.noUpdate) {
			Assert.assertEquals(0, openedFileSpecs.size());
		} else {
			helper.validateFileSpecs(addedFileSpecs);
			Assert.assertEquals(true, openedFileSpecs.size() > 0);
			for (IFileSpec openedFileSpec : openedFileSpecs) {
				Assert.assertEquals(valids.bypassClientUpdate, !testFile.canWrite());
				Assert.assertEquals(valids.changelistId, openedFileSpec.getChangelistId());
				Assert.assertEquals(valids.fileType, openedFileSpec.getFileType());
			}
		}
	}
