...
public interface P4ServerExec {
    ...
    /**
     * Perform a query that requires an in-thread response.
     * These should generally be avoided except where the calling
     * IDEA API requires an immediate response.
     *
     * @param query description and parameters of the query
     * @param <R> result type of the query
     * @return the result for the query.
     * @throws ServerResultException when the query generates an error.
     */
    @NotNull
    <R extends ServerResult> ServerResult syncQuery(@NotNull ServerConfig config, @NotNull SyncServerQuery<R> query)
        throws ServerResultException;

    @NotNull
    <R extends ClientResult> ClientResult syncQuery(@NotNull ClientConfig config, @NotNull SyncClientQuery<R> query)
            throws ServerResultException;
}
