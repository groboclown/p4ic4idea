...
public class Job100498Test extends P4JavaRshTestCase {
	...
	public void testCheckSymlinkWithRestrictSymlinkEnabled() throws Exception {
		...
		List<String> errors = getErrorsFromFileSpecList(files);
		assertNotNull(errors);
		assertTrue(errors.size() == 1);
		assertTrue(errors.get(0).contains("is not inside permitted filesystem path"));

		// Check that some files were successful in the sync
		List<String> valids = getMessagesFromFileSpecList(FileSpecOpStatus.VALID, files);
