...
public class LogoutDelegatorTest extends AbstractP4JavaUnitTest {
    ...
    private void setUp(final Class<? extends P4JavaException> exceptionClass)
        ...
        when(server.execMapCmdList(eq(LOGOUT.toString()), any(String[].class), (Map<String,Object>)any()))
    ...
    public void testLogout() throws P4JavaException {
        when(server.execMapCmdList(eq(LOGOUT.toString()), argThat(EMPTY_MATCHER), eq((Map<String, Object>)null)))
                .thenReturn(null);
        when(server.getAuthTicket()).thenReturn("auth");
        logoutDelegator.logout();
        verify(server).execMapCmdList(eq(LOGOUT.toString()), argThat(EMPTY_MATCHER), eq((Map<String, Object>)null));
