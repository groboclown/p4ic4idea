...
public class GroupsDelegatorTest extends P4JavaTestCase {
    ...
    public void testConnectionException() throws Exception {
        when(server.execMapCmdList(eq(CmdSpec.GROUPS.toString()),
                argThat(new CommandLineArgumentMatcher(new String[] { "-i", "-v", groupName })),
                eq((Map<String, Object>) null)))
                        .thenThrow(new ConnectionException("Read timed out."));
    ...
    public void testAccessException() throws P4JavaException {
        when(server.execMapCmdList(eq(CmdSpec.GROUPS.toString()),
                argThat(new CommandLineArgumentMatcher(new String[] { "-i", "-v", groupName })),
                eq((Map<String, Object>) null))).thenThrow(new AccessException("Denied."));
        groupsDelegator.getUserGroups(groupName, true, true, -1);
    }
    ...
    public void testRequestException() throws P4JavaException {
        when(server.execMapCmdList(eq(CmdSpec.GROUPS.toString()),
                argThat(new CommandLineArgumentMatcher(new String[] { "-i", "-v", groupName })),
                eq((Map<String, Object>) null))).thenThrow(new RequestException("Invalid"));
        groupsDelegator.getUserGroups(groupName, true, true, -1);
    }
    ...
    public void testGetMany() throws P4JavaException {
        ...
        when(server.execMapCmdList(eq(CmdSpec.GROUPS.toString()),
                ...
                eq((Map<String, Object>) null))).thenReturn(populatedResultMaps);
        when(server.getErrorStr((Map<String, Object>) any())).thenReturn(EMPTY);
    ...
    @SuppressWarnings({ "unchecked", "deprecation" })
    @Test
    public void testUnlimitedTimeout() throws P4JavaException {
        ...
        /*
         * when the a bug is fixed assertEquals(IUserGroup.UNLIMITED,
         * userGroups.get(0).getTimeout());
         */
    }
