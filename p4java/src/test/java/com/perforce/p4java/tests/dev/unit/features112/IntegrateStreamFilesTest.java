 ...
 */
@Jobs({ "job046698" })
@TestId("Dev112_IntegrateStreamFilesTest")
public class IntegrateStreamFilesTest extends P4JavaTestCase {
	...
	public void testIntegStreamFiles() {
		...
		try {
			...
			changelist = getNewChangelist(server, mainStreamClient,
					"Dev112_IntegrateStreamFilesTest copy files");
			...
			changelist = getNewChangelist(server, devStreamClient,
					"Dev112_IntegrateStreamFilesTest integ files");
			assertNotNull(changelist);
			...
			files = devStreamClient.integrateFiles(null,
			...
			if (server != null) {
				if (devStreamClient != null) {
					try {
						...
						server.setCurrentClient(devStreamClient);
						changelist = getNewChangelist(server, devStreamClient,
								"Dev112_IntegrateStreamFilesTest delete submitted files");
