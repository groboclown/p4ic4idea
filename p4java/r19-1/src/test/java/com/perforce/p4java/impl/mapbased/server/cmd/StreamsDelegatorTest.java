...
public class StreamsDelegatorTest extends AbstractP4JavaUnitTest {
    ...
    public void beforeEach() throws Exception {
        ...
        streamPaths = new ArrayList<String>();
        for (String p : STREAM_PATHS) {
            streamPaths.add(p);
        }

        when(server.execMapCmdList(eq(STREAMS.toString()), eq(CMD_ARGUMENTS),
    ...
    public void tesGetStreamsShouldReturnEmptyStreamSummariesWhenResultMapsIsNull()
        ...
        getStreamShouldReturnEmptyList(new UnitTestGiven() {
            @Override
    ...
    /**
     * Expected return empty stream summaries all result map's file error string
     * is not blank
     *
     * @throws Exception
     */
