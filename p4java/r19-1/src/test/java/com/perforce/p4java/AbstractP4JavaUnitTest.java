...
public abstract class AbstractP4JavaUnitTest {
	...
	protected void testIfGivenExceptionWasThrown(Class<? extends P4JavaException> firstThrownException, Class<? extends P4JavaException> expectedThrows, UnitTestGivenThatWillThrowException unitTestGiven) throws P4JavaException {
		try {
		    unitTestGiven.given(firstThrownException);
		    Assert.fail("Expected exception [" + expectedThrows.getName() + "] to be thrown, but no exception was thrown" );
		} catch( Throwable e ) {
		    Assert.assertEquals(expectedThrows, e.getClass());
		}
	}

	protected void testIfGivenExceptionWasThrown(Class<? extends P4JavaException> firstThrownException, UnitTestGivenThatWillThrowException unitTestGiven) throws P4JavaException {
		testIfGivenExceptionWasThrown(firstThrownException, firstThrownException, unitTestGiven);
	}
	...
	protected void verifyFileSizeAfterSyncFromDepot(
		...
		Assert.assertTrue(fileDepotPath, fileSpec.getDepotPathString().contains(fileDepotPath));
	...
	protected SubmittingSupplier submitFileThatLoadFromClassPath(
		...
		Assert.assertTrue(file.getDepotPathString().contains(fileDepotPath));
