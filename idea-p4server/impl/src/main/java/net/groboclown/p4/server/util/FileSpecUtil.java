...
import net.groboclown.p4.server.api.exceptions.P4ServerException;
import net.groboclown.p4.server.api.exceptions.P4FileException;
...
public class FileSpecUtil {
     ...
     */
    @NotNull
    static IFileSpec getOneSpec(@NotNull String path) throws P4ServerException {
    ...
    @NotNull
    public static IFileSpec getOneSpecWithRev(@NotNull FilePath file, int rev) throws P4ServerException {
        if (file.isDirectory()) {
        ...
        if (spec.size() != 1) {
            throw new P4ServerException(P4Bundle.message("error.annotate.multiple-files", path, spec));
        }
        return spec.get(0);
    }
    ...
    @NotNull
    public static IFileSpec getFromFilePath(@NotNull FilePath file) throws P4ServerException {
        return getFromFilePathsAt(Collections.singletonList(file), "", false).get(0);
    }


    @NotNull
    public static List<IFileSpec> getFromFilePaths(@NotNull Collection<? extends FilePath> files) throws
            P4ServerException {
        return getFromFilePathsAt(files, "", false);
    }

    @NotNull
    public static List<IFileSpec> getFromVirtualFiles(@NotNull Collection<VirtualFile> files) throws P4ServerException {
     ...
     * @return specs
     * @throws P4ServerException if there was an invalid file path
     */
    @NotNull
    ...
    @NotNull
    public static List<IFileSpec> getFromVirtualFilesAt(@NotNull Collection<VirtualFile> files, @NotNull String revisionPart, boolean allowDirectories) throws
            P4ServerException {
        if (files.isEmpty()) {
            return Collections.emptyList();
        }
    ...
    /** Use with care... */
    @NotNull
    public static List<IFileSpec> makeRootFileSpecs(@Nullable VirtualFile[] roots) throws P4ServerException {
        if (roots == null) {
            return Collections.emptyList();
        }
