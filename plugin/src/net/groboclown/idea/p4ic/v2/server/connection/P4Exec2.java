...
public class P4Exec2 {
    ...
    public P4Exec2(@NotNull ServerStatus serverStatus, @Nullable String clientName,
     ...
     */
    @NotNull
    public MessageResult<List<IFileSpec>> loadOpenedFiles(@NotNull Project project,
            @NotNull final List<IFileSpec> openedSpecs, final boolean fast)
            throws VcsException, CancellationException {
        LOG.debug("loading open files " + openedSpecs);
        return runWithClient(project, new WithClient<MessageResult<List<IFileSpec>>>() {
            @Override
     ...
     */
    @NotNull
    public List<IExtendedFileSpec> getFileStatus(@NotNull Project project, @NotNull final List<IFileSpec> files)
            throws VcsException, CancellationException {
        return runWithClient(project, new WithClient<List<IExtendedFileSpec>>() {
            @Override
            public List<IExtendedFileSpec> run(@NotNull final IOptionsServer server,
                    @NotNull final IClient client, @NotNull final ServerCount count)
                    throws P4JavaException, IOException, InterruptedException, TimeoutException, URISyntaxException,
                    P4Exception {
    ...
    */
    ...
    private <T> T p4RunFor(@NotNull Project project, @NotNull P4Runner<T> runner, boolean triedLogin)
        ...
        // FIXME
        // FIXME
        // HUGE fixme on this guy.  The only reason to loop here is if the password needs to be set by
        // the connection.  Because of this, this should be called out to another method.
