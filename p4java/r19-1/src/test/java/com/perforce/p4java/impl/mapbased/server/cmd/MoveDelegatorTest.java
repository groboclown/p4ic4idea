...
public class MoveDelegatorTest extends AbstractP4JavaUnitTest {
    ...
    private String[] moveCmdArguments = { "-c" + CHANGELIST_ID, "-t" + FILE_TYPE,
            FROM_FILE_PATH_STRING, TO_FILE_PATH_STRING };
    ...
    public class TestMoveFile {
        /**
         * Test moveFile(CHANGELIST_ID, LIST_ONLY, noClientMove, FILE_TYPE,
         * fromFileSpec, toFile)
         */
        public class WhenChangelistIdListOnlyNoClientMoveFileTypeFromFileToFileGiven {
            ...
            /**
             * Expected throws <code>NullPointerException</code> when fromFile
             * is null, So precondition check is fail.
             *
             * @throws Exception
             */
            @Test
            ...
            /**
             * Expected throws <code>NullPointerException</code> when fromFile's
             * preferredPath is null, So precondition check is fail.
             *
             * @throws Exception
             */
            @Test
            ...
            /**
             * Expected throws <code>NullPointerException</code> when toFile is
             * null, So precondition check is fail.
             *
             * @throws Exception
             */
            @Test
            ...
            /**
             * Expected throws <code>NullPointerException</code> when toFile's
             * preferredPath is null, So precondition check is fail.
             *
             * @throws Exception
             */
            ...
            /**
             * Expected throws <code>RequestException</code> when server version
             * is less than 20091 as it's not support 'move file'
             *
             * @throws Exception
             */
            @Test
            public void shouldThrownRequestExceptionWhenServerVersionLessThan20091()
                    throws Exception {
                // given
                serverVersion = 20090;
            ...
            /**
             * Expected throws <code>RequestException</code> when 'noClientMove'
             * not support by server version is less than 20092 as it's not
             * support 'move file'
             *
             * @throws Exception
             */
            @Test
            public void shouldThrownRequestExceptionWhenNoClientMoveOptionNotSupportedByServerVersionLessThan20092()
                    throws Exception {
                // given
                serverVersion = 20091;
                when(fromFileSpec.getPreferredPath()).thenReturn(mock(FilePath.class));
            ...
            private void executeAndVerifyExpectedPreconditionFailException(
                    Class<? extends Throwable> expectedThrownException) throws Exception {

                thrown.expect(expectedThrownException);

                moveDelegator.moveFile(CHANGELIST_ID, LIST_ONLY, noClientMove, FILE_TYPE,
                        fromFileSpec, toFileSpec);

                verify(server, never()).execMapCmdList(eq(MOVE.toString()), any(String[].class),
                        eq((Map<String, Object>) null));
            }

            /**
             * Expected throws <code>ConnectionException</code> or
             * <code>ConnectionException</code> when inner method call throws it
             *
             * @throws Exception
             */
            @Test
            ...
            /**
             * Expected return empty moved file list when inner method call
             * throws <code>P4JavaException</code>.
             *
             * @throws Exception
             */
            @Test
            public void shouldReturnEmptyListWhenInnerMethodCallThrowsP4JavaException()
                    throws Exception {
                // when
                List<IFileSpec> fileSpecs = executeThenExpectThrownExceptionOrEmptyList(
            ...
            private List<IFileSpec> executeThenExpectThrownExceptionOrEmptyList(
                    Class<? extends Throwable> expectedThrownException) throws Exception {
                when(server.getServerVersion()).thenReturn(serverVersion);
                doThrow(expectedThrownException).when(server).execMapCmdList(eq(MOVE.toString()),
                        eq(moveCmdArguments), eq((Map<String, Object>) null));
                return moveDelegator.moveFile(CHANGELIST_ID, LIST_ONLY, noClientMove, FILE_TYPE,
                        fromFileSpec, toFileSpec);
            }
            ...
            public void shouldReturnNonEmptyMovedFileSpecs() throws Exception {
                ...
                when(server.execMapCmdList(eq(MOVE.toString()), eq(moveCmdArguments),
                        eq((Map<String, Object>) null))).thenReturn(resultMaps);
                // when
                List<IFileSpec> fileSpecs = moveDelegator.moveFile(CHANGELIST_ID, LIST_ONLY,
                        noClientMove, FILE_TYPE, fromFileSpec, toFileSpec);
        ...
        public class WhenFromFileToFileMoveFileOptionsGiven {
            ...
            @Before
            public void beforeEach() {
                moveFileOptions = new MoveFileOptions(CHANGELIST_ID, LIST_ONLY, FORCE, noClientMove,
                        FILE_TYPE);
            }
            ...
            private void executeAndVerifyExpectedPreconditionFailException(
                ...
                moveDelegator.moveFile(fromFileSpec, toFileSpec, moveFileOptions);
            ...
            public void shouldReturnNonEmptyMovedFileSpecs() throws Exception {
                ...
                when(server.execMapCmdList(eq(MOVE.toString()), eq(moveCmdArguments),
                        eq((Map<String, Object>) null))).thenReturn(resultMaps);
                List<IFileSpec> fileSpecs = moveDelegator.moveFile(fromFileSpec, toFileSpec,
                        moveFileOptions);
                // then
                assertThat(fileSpecs.size(), is(1));
                assertThat(fileSpecs.get(0).getFromFile(), is(FROM_FILE_PATH_STRING));
