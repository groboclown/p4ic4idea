...
public class ChangeDelegatorTest extends AbstractP4JavaUnitTest {
    ...
    private static final CommandLineArgumentMatcher DELETE_ID_PARAMS_MATCHER = new CommandLineArgumentMatcher(
            DELETE_ID_PARAMS);

    /** Matcher for calls to get by id. */
    private static final CommandLineArgumentMatcher GET_ID_PARAMS_MATCHER = new CommandLineArgumentMatcher(
            GET_ID_PARAMS);

    /** Matcher for calls to get by default id. */
    private static final CommandLineArgumentMatcher DEFAULT_ID_PARAMS_MATCHER = new CommandLineArgumentMatcher(
            DEFAULT_PARAMS);
    ...
    public void testGetChangelistConnectionException() throws Exception {
        when(server.execMapCmdList(eq(CHANGE.toString()), any(String[].class),
    ...
    public void testGetChangelistbyId() throws P4JavaException {
        when(server.execMapCmdList(eq(CHANGE.toString()), argThat(GET_ID_PARAMS_MATCHER),
                eq((Map<String, Object>) null))).thenReturn(buildChangeList());
    ...
    public void testGetChangelistbyIdAndOptions() throws Exception {
        ...
        verify(server).execMapCmdList(eq(CHANGE.toString()), argThat(GET_ID_PARAMS_MATCHER),
                eq((Map<String, Object>) null));
        assertNull("Change list returned should have been null", cl);
    }
    ...
    public void testGetChangelistbyIdAndOptionsNull() throws Exception {
        when(server.execMapCmdList(eq(CHANGE.toString()), argThat(GET_ID_PARAMS_MATCHER),
                eq((Map<String, Object>) null))).thenReturn(buildNullChangeList());
        IChangelist cl = changeDelegator.getChangelist(MOCK_LIST_ID, mockChangelistOptions);
        verify(server).execMapCmdList(eq(CHANGE.toString()), argThat(GET_ID_PARAMS_MATCHER),
                eq((Map<String, Object>) null));
        assertNull("Change list returned should have been null", cl);
    ...
    public void testDeletePendingChangelist() throws Exception {
        ...
        when(server.execMapCmdList(eq(CHANGE.toString()), argThat(DELETE_ID_PARAMS_MATCHER),
                eq((Map<String, Object>) null)))
                        .thenReturn(buildChangeList(MESSAGE_CODE_IN_INFO_RANGE));
