 ...
 */
public class ChangeListSyncNew {
    private static final Logger LOG = Logger.getInstance(ChangeListSyncNew.class);


    private final P4Vcs vcs;
    private final Project project;

    public ChangeListSyncNew(@NotNull final P4Vcs vcs) {
        this.vcs = vcs;
                        ...
                        if (revChangeList == null) {
                            ...
                            if (client != null) {
                                ...
                                if (change.getAfterRevision() == null) {
                                    LOG.info("processing locally deleted file " + revFile);
                                    if (change.getBeforeRevision() == null) {
                                        LOG.error("Both the before and after revisions are null for change " + change);
                                    } else {
                                        builder.processLocallyDeletedFile(change.getBeforeRevision().getFile());
                                    }
                                } else if (change.getBeforeRevision() == null) {
                                    LOG.info("processing locally added file " + revFile);
                                    if (change.getAfterRevision().getFile().getVirtualFile() == null) {
                                        LOG.info("processing after revision deleted (no before revision) " + change
                                                .getAfterRevision().getFile());
                                        builder.processLocallyDeletedFile(change.getAfterRevision().getFile());
                                    } else {
                                        LOG.info("processing unmanaged new file " + change.getAfterRevision().getFile());
                                        builder.processUnversionedFile(
                                                change.getAfterRevision().getFile().getVirtualFile());
                                    }
                                } else if (revFile.getVirtualFile() != null) {
                                    LOG.info("processing modified without checkout " + revFile);
                                    builder.processModifiedWithoutCheckout(revFile.getVirtualFile());
                                } else {
                                    LOG.info("processing weirdly deleted file " + revFile);
